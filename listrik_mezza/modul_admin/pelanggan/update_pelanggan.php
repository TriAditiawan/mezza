<?php
	include "koneksi.php";
	$kode= $_GET['kode'];
	$queryEdit = mysqli_query($connection,"SELECT * FROM tbl_pelanggan where id_pelanggan='$kode' limit 1")or die(mysqli_error());
	$dataEdit = mysqli_fetch_array($queryEdit);
?>
<div id="konten">
  <h1>Data Pelanggan</h1>
  <form method="POST"
    action="media_admin.php?module=update_proses_pelanggan&amp;kode=<?php echo $dataEdit['id_pelanggan'];?>"
    ...
    <table cellpadding="0" cellspacing="0" border="0">
      <tr>
        ...
        <td>:</td>
        <td>
          <input type="text" name="input_id_pelanggan" size="40%" value=<?php echo $dataEdit['id_pelanggan'];?>>
        </td>
      </tr>
      <tr>
        <td width="20%">Nomor Meter</td>
        <td>:</td>
        <td><input type="text" name="input_no_meter" size="40%" value=<?php echo $dataEdit['no_meter'];?>></td>
      </tr>
      <tr>
        <td width="20%">Nama Pelanggan</td>
        <td>:</td>
        <td><input type="text" name="input_nama" size="40%" value=<?php echo $dataEdit['nama'];?>></td>
      </tr>
      <tr>
        <td width="20%">Alamat Pelanggan</td>
        <td>:</td>
        <td><input type="text" name="input_alamat" size="40%" value=<?php echo $dataEdit['alamat'];?>></td>
      </tr>
      <tr>
        <td width="20%">Daya</td>
        <td>:</td>
      ...
      <tr>
        <td></td>
        <td></td>
        <td><input type="submit" name="tambah pelanggan" value="Update Pelanggan">
