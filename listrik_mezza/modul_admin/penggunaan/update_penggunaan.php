<?php
	include "koneksi.php";
	$kode= $_GET['kode'];
	$queryEdit = mysqli_query($connection,"SELECT * FROM tbl_penggunaan where id_penggunaan='$kode' limit 1")or die(mysqli_error());
	$dataEdit = mysqli_fetch_array($queryEdit);
?>
<div id="konten">
  <h1>Data penggunaan</h1>
  <form method="POST"
    action="media_admin.php?module=update_proses_penggunaan&amp;kode=<?php echo $dataEdit['id_penggunaan'];?>"
    ...
    <table cellpadding="0" cellspacing="0" border="0">
      <tr>
        ...
        <td>:</td>
        <td>
          <input type="text" name="input_id_penggunaan" size="40%" value=<?php echo $dataEdit['id_penggunaan'];?>>
        </td>
      </tr>
      <tr>
        <td width="20%">Daya</td>
        <td>:</td>
        <td>
          <select name="input_id_pelanggan" style="width: 70%;">
      ...
      <tr>
        ...
        <td>:</td>
        <td>
          <input type="text" name="input_meter_awal" size="40%" value=<?php echo $dataEdit['meter_awal'];?>>
        </td>
      </tr>
      <tr>
        ...
        <td>:</td>
        <td>
          <input type="text" name="input_meter_akhir" size="40%" value=<?php echo $dataEdit['meter_akhir'];?>>
        </td>
      </tr>
      <tr>
        <td></td>
        <td></td>
        <td><input type="submit" name="update_penggunaan" value="Update">
